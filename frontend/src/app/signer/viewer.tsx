    ...
    <section className="h-screen">
      <div className="flex bg-white p-2 border-b border-l border-(--line)">
        <div className="p-1 flex">
          <button className="p-1" onClick={() => setPage((e) => (e+1))}>+</button>
          <p className="m-1 mx-2">{page}</p>
          <button className="p-1" onClick={() => setPage((e) => (e-1))}>-</button>
        </div>
        {barcode ? (
          ...
          // <button onClick={async () => (await sendFileToBE(path, x, y))} className="mx-1 px-5 py-2 bg-(--secondary) rounded-xl border border-(--line)">Tandatangani dokumen</button>
        ) : (
      ...
      <div className="w-full overflow-scroll flex justify-center items-center bg-gray-100 border-l border-(--line)">
        <div className="relative scale-75">
          <div style={{width: `${PointToPixel(width)}px`, height: `${PointToPixel(height)}px`, background: "red", boxShadow: "1px 1px 10px 4px var(--line)"}} ref={pageRef} onClick={handleClick}>
      ...
      {modal && (
        <Modal >
          <div ref={modalRef} className="w-[50%] h-[80%] bg-red-200"></div>
        </Modal>
      )}
    </section>
  );
